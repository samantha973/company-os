...
export function DealManage({ deal, stages }: { deal: DealManageData; stages: DealStage[] }) {
  ...
  // stage move and re-synced from the server after router.refresh(). Everything
  // else (money, terms, referrers, activity) reads from props / inline editors.
  const [stageId, setStageId] = useState(deal.stageId);
  const [status, setStatus] = useState(deal.status ?? "open");
  const [handoffStatus, setHandoffStatus] = useState(deal.handoffStatus);
  const [archived, setArchived] = useState(!!deal.archivedAt);
  ...
  async function moveToStage(stage: DealStage, opts?: { lostReason?: string; wonAmount?: number }) {
    ...
    setStageId(stage.id);
    setStatus(stage.isWon ? "won" : stage.isLost ? "lost" : "open");
    if (wasPending) setHandoffStatus("accepted");
    ...
    if (!r.ok) {
      ...
      setHandoffStatus(deal.handoffStatus);
    } else {
      router.refresh();
    }
  }
