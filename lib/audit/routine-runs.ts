...
// lib/ai/response.ts reports every model call's usage into whichever run is
 ...
 * It lives here rather than in lib/ai/response.ts because it describes what
...
/** Called by lib/ai/response.ts for every model call; a no-op outside a run. */
...
export async function withRoutineRun(
  ...
  return recordRoutineRun(routineId, req, handler, host);
}

/**
 * Record a run without the cron bearer gate, for routines that are triggered
 * by a verified inbound event (a signed webhook) rather than by Vercel Cron.
 * The handler keeps its own authentication; a 401 it returns is not recorded.
 */
