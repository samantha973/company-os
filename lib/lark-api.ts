...
// Minutes metadata for one recording token.
export async function fetchMinutesMeta(token: string): Promise<MinutesMeta | null> {
  const res = await larkFetch(`/open-apis/minutes/v1/minutes/${token}`, { method: "GET" });
  if (!res) return null;
  const json = (await res.json()) as {
    code: number;
    msg?: string;
    data?: { minute?: { title?: string; start_time?: string | number } };
  };
  if (json.code !== 0) {
    console.error(`[lark-api] minutes meta ${token} failed:`, json.code, json.msg);
    return null;
  }
  const m = json.data?.minute;
  const start = m?.start_time != null ? new Date(Number(m.start_time)).toISOString() : null;
  return { token, title: m?.title ?? null, startTime: start };
}
