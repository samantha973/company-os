...
export async function sendAdminSelfServeSignInLink(rawEmail: string): Promise<void> {
  ...
  const { data, error } = await supabase.auth.admin.generateLink({
    type: "magiclink",
    email,
    options: { redirectTo: `${siteOrigin()}/api/auth/callback?next=/admin` },
  });
  const tokenHash = data?.properties?.hashed_token;
  if (error || !tokenHash) return;
  const verifyUrl = `${siteOrigin()}/admin/verify?token_hash=${encodeURIComponent(tokenHash)}&type=magiclink`;
