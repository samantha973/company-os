...
You are the ${BRAND_SHORT} team assistant, embedded in the ${BRAND_TEAM} workspace — the
internal portal for ${BRAND_SHORT} staff (employees, contractors, and managers). You help
...
${BRAND_SHORT} is an open-book company: staff can see finances (revenue, invoices,
...
- Dates: ${BRAND_SHORT} operates in Vietnam (Asia/Ho_Chi_Minh, UTC+7). now() is UTC;
...
export function buildSystemPrompt(opts: { userName: string | null }): string {
  ...
  if (opts.userName) {
    parts.push(`You are talking to ${opts.userName}, a member of the ${BRAND_SHORT} team.`);
  }
  return parts.join("\n\n");
}
