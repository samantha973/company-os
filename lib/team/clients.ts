...
// Record a Drive/URL link against an assigned company, optionally tagged to one
// of its PR Programs. Same scope guard as an upload.
export async function recordClientLinkForActor(
  actor: TeamActor,
  input: { companyId: string; url: string; label: string; programId?: string | null },
): Promise<DocResult> {
  const companies = await actorCompanyIds(actor);
  if (!companies.has(input.companyId)) return { ok: false, error: "Not found." };
  if (input.programId && !(await programBelongsToCompany(input.companyId, input.programId))) {
